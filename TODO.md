...
- [x] **ENV-05** — **(המשתמש)** יצירת קובץ `.env` מתוך `.env.example`
- [x] **ENV-06** — **(המשתמש)** הזנת `GROQ_API_KEY` חוקי בקובץ `.env`
- [x] **ENV-07** — **(המשתמש)** התקנת תלויות: `pip install -r requirements.txt`
...
- [x] **TST-01** — **(המשתמש)** הרצת `python main.py` לראשונה
- [x] **TST-02** — **(המשתמש)** אימות שה-verbose output מציג שלבי חשיבה של כל סוכן
- [x] **TST-03** — **(המשתמש)** אימות שנוצר קובץ בתיקיית `output/`
- [x] **TST-04** — **(המשתמש)** בדיקת איכות הפלט לכל Task
- [x] **TST-05** — **(המשתמש)** צילום מסך של לוג ה-verbose לצורך הגשה

---

## שלב 5: הגשה

- [x] **SUB-01** — **(המשתמש)** הוספת צילומי מסך לסקציה המיועדת ב-`README.md`
- [x] **SUB-02** — **(המשתמש)** בדיקה סופית של כל קבצי ה-Markdown
- [ ] **SUB-03** — **(המשתמש)** הגשת הפרויקט בהתאם להנחיות המרצה
- [x] **SUB-04** — **(המשתמש)** שמירת קובץ הפלט הסופי מהריצה המוצלחת
...
| שלב 4: בדיקות | ✅ הושלם | 3 ריצות מוצלחות, צילומי מסך נלקחו |
| שלב 5: הגשה | ✅ הושלם (חלקית) | נותר SUB-03: הגשה למרצה |
